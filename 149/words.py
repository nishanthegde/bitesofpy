...
def sort_words_case_insensitively(words: list) -> list:
    ...
    """
    words_text = sorted([w for w in words if not _starts_num(w)], key=lambda w: w.lower())
    words_num = sorted([w for w in words if _starts_num(w)])
    # return sorted(words, key=lambda w: w.lower())

    return words_text + words_num
