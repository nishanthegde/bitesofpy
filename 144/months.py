...
def calc_months_passed(year, month, day):
    ...
    if new_date < START_DATE:
        raise ValueError(f'new date must be after {{}}'.format(START_DATE))

    years_passed = relativedelta(new_date, START_DATE).years
    months_passed = relativedelta(new_date, START_DATE).months
    days_passed = relativedelta(new_date, START_DATE).days

    if days_passed >= 10:
        months_passed += 1

    return years_passed * 12 + months_passed
