...
def fix_translation(org_text, trans_text):
    ...
    """
    # replace content in code tags
    code_to_be_replaced = []

    code_tag_matches = re.findall(r'<code.*?>([\s\S]*?)</code>', trans_text, re.MULTILINE)

    for match in code_tag_matches:
        code_to_be_replaced.append(match)

    code_replacements = []

    code_tag_matches = re.findall(r'<code.*?>([\s\S]*?)</code>', org_text, re.MULTILINE)

    for match in code_tag_matches:
        code_replacements.append(match)

    trans_text = re.sub(r'(<code.*?>)([\s\S]*?)(</code>)', r'\1{}\3', trans_text).format(*code_replacements)

    # replace content in pre tags
    pre_to_be_replaced = []

    pre_tag_matches = re.findall(r'<pre.*>([\s\S]*?)</pre>', trans_text, re.MULTILINE)

    for match in pre_tag_matches:
        pre_to_be_replaced.append(match)

    pre_replacements = []

    pre_tag_matches = re.findall(r'<pre.*>([\s\S]*?)</pre>', org_text, re.MULTILINE)

    for match in pre_tag_matches:
        pre_replacements.append(match)

    trans_text = re.sub(r'(<pre.*>)([\s\S]*?)(</pre>)', r'\1{}\3', trans_text).format(*pre_replacements)

    # return list(zip(code_to_be_replaced, code_replacements))
    return trans_text


# def main():
#     # print(type(fix_translation(bite_15_en, bite_15_it)))
#     # print(fix_translation(bite_15_en, bite_15_it))
#     assert fix_translation(bite_15_en, bite_15_it) == bite_15_it_fixed
#     assert fix_translation(bite_202_en, bite_202_de) == bite_202_de_fixed
#     assert fix_translation(bite_209_en, bite_209_es) == bite_209_es_fixed
