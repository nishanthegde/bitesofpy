...
def check_neighbors_val1(grid_list: list, val: int, center: tuple) -> tuple:
    # check 4 neighbors for next value and return tuple of indices of new center and direction
    ...
    if center_col_idx < len(grid_list) - 1 and grid_list[center_row_idx][center_col_idx + 1] == val + 1:
        center_col_idx = center_col_idx + 1
        direction = RIGHT
    if center_col_idx > 0 and grid_list[center_row_idx][center_col_idx - 1] == val + 1:
        center_col_idx = center_col_idx - 1
        direction = LEFT
    if center_row_idx < len(grid_list[0]) - 1 and grid_list[center_row_idx + 1][center_col_idx] == val + 1:
        center_row_idx = center_row_idx + 1
        direction = DOWN
    if center_row_idx > 0 and grid_list[center_row_idx - 1][center_col_idx] == val + 1:
        center_row_idx = center_row_idx - 1
        direction = UP
...
def print_sequence_route(grid, start_coordinates=None):
    ...
    for i, e in enumerate(elements):
        ...
        for k, v in enumerate(grid_list):
